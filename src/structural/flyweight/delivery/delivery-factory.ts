...
export class DeliveryFactory{
    ...
    makeLocation (intrisecState: DeliveryLocationData): DeliveryFlyWeight {
        const key = this.createId(intrisecState)
        if (key in this.locations) return this.locations[key]
        this.locations[key] = new DeliveryLocation(intrisecState)
